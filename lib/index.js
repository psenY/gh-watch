  ...
  const deliver = (agent, msg, label) => {
    try {
      if (agent && agent.status === 'running' && typeof agent.steer === 'function') {
        agent.steer(msg)
        console.log('[gh-watch] steered ' + label + '（插话，当前轮进行中）')
        return
      }
      if (typeof agent.followup === 'function') { agent.followup(msg); console.log('[gh-watch] notified ' + label) }
      else if (typeof agent.steer === 'function') { agent.steer(msg); console.log('[gh-watch] steered ' + label) }
    } catch (e) {
    ...
    try {
      if (useNew) {
        ...
        if (!agent) {
          ...
          const h = await agents.resume({ resumeSessionId: ownerSessionId, ...defaultAgentOptions(ctx) })
          agent = h.agent
        }
