...
export function apply(ctx, config) {
  ...
  async function dispatch(ownerSessionId, w, change) {
    ...
    // 消息必须带 source（对齐 dsh-qqbot 的 createUserMessage 结构）：
    // AgentTeams 等消费方会读 message.source.kind，缺 source 会抛
    // "Cannot read properties of undefined (reading 'kind')" 导致通知投递崩溃。
    const message = { content, source: { kind: 'plugin', plugin: 'gh-watch' } }
